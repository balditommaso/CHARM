...
def gen_aie_top(prj_dir, template_dir, Model_MM, placement):
    ...
    # Copy Data
    data_dir = template_dir + "/data/kernel" + str(Model_MM[0][11])   # TODO: support multiple kernels
    ...
    for i in range(HW_Conf.shape[0]):
        Port_Conf_All[i][0] = (
            HW_Conf[i][0]
            * (HW_Conf[i][2] // HW_Conf[i][3])
            * (HW_Conf[i][1] // HW_Conf[i][5])
        )  # lhs_port

        Port_Conf_All[i][1] = (
            HW_Conf[i][2]
            * (HW_Conf[i][0] // HW_Conf[i][4])
            * (HW_Conf[i][1] // HW_Conf[i][5])
        )  # rhs_port

        Port_Conf_All[i][2] = (
            HW_Conf[i][0] * HW_Conf[i][2] // HW_Conf[i][6]
        )  # out_port
    ...
    for i in L_list:
        ...
        gen_krnl(
            environment_kernel[kernel_type],
            w1,
            kernel_type,
            layer_name,
            aie_folder,
