def gen_para(environment, h1, w1, w2, A, B, C, A_BRO, C_BRO,
             PACK_IN, data_type, layer, aie_folder):
...
def gen_krnl(environment, w1, kernel_type, layer, aie_folder):
    ...
    for i in range(4):
        ...
        kernel_name = f"mm{kernel_type}_kernel{i}.cc"
        template = environment.get_template(kernel_name)

        file_tmp = f"mm{kernel_type}_kernel{i}_L{layer}.cc"
        filename = aie_folder / file_tmp
...
def gen_grah(environment, B, layer, aie_folder):
    ...
    file_tmp = f"aie_graph_L{layer}.h"
    filename = aie_folder / file_tmp
