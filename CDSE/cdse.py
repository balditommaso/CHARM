...
def cdse_top(Op0, Op1):
    ...
    retrun:
    fianl_config: final configuration of the framework.
    '''
    ...
    # define the datatype of the matrices
        ...
        for b in range(1, 50 + 1): ##Col Constaint
            for a in range(1, AIE_NUM // (c * b) + 1): 
                if (force_assign == 1) and ((a != A) or (b != B) or (c != C)):
                    continue
                # if type is int 16 a must be even
                if (DATA_TYPE == 2) and (a % 2 != 0):
                  continue
              
                # different bradcast strategy base on the kernel type
                if kernel_type % 2 == 1:
                    A_BRO = c // 2
                    height = min(c,8)
                    if a % 2 == 0:
                ...
                # check if the PLIO counts respect the limit
                ...
                # nested loop to iterate over possible design implementations
                for x in range(1, 16):
                    for y in range(1, 16): 
                        for z in range(1, 16):
                            ...
                            for large_t in range(sample_num):
                                ...
                                AIE_CYCLE = math.ceil(
                                    ...
                                    ) * x * y * z + (H1 * W1 * DATA_TYPE // 4) * PACK_IN + (H1 * W2 * DATA_TYPE // 4) * PACK_OUT  
                                ...
                                if Y_TILE==1 and X_TILE==1 and Z_TILE==1:
                                    ...
                                    temp_cycle[0, large_t] = max([load_L_DR,load_R_DL]) + \
