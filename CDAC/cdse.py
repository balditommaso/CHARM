...
def cdse_top(MODEL_IN, HW_Part, DATA_TYPE):
    ...
    # One-Time Profiling of DDR Bandwidth
    ...
    num_design_best = 50
    num_design_choice = 50

    temp_cycle = np.zeros([1, sample_num])
    config = np.ones([num_design_best + num_design_choice,
                      num_term + sample_num]) * 1e30
                ...
                for x in range(1, 16):
                    for y in range(1, 16):
                        for z in range(1, 16):
                            ...
                            bram_use, uram_use, buf_index = buff_count_0(
                                BRAM, URAM,
                                PART_A, PART_B, PART_C,
                                PACK_IN, PACK_OUT,
                                LEFT_SIZE, RIGHT_SIZE, OUT_SIZE,
                                a, b, c, x, y, z,
                                DBUFF_L, DBUFF_R, DBUFF_O,
                                RAM_TYPE_A, RAM_TYPE_B, RAM_TYPE_C,
                                DATA_TYPE, force_assign, buf_sel
                            )
                            ...
                            for large_t in range(sample_num):
                                ...
                                if X_TILE == 1 and Y_TILE == 1 and Z_TILE == 1:
                                    temp_cycle[0, large_t] = (
                                        max([load_L_DR, load_R_DL]) +
                                        AIE_CYCLE +
                                        store_O_S
                                    )
                                    ...
                                    temp_cycle[0, large_t] = (
                                        max([load_L_DR, load_R_DL])
                                        + max([load_L_DR, load_R_DL, AIE_CYCLE])
                                        * ((X_TILE * Y_TILE * Z_TILE - 1)
                                           - (X_TILE * Z_TILE - 1))
                                        + max([load_L_T, load_R_T,
                                               AIE_CYCLE, store_O_T])
                                        * (X_TILE * Z_TILE - 1)
                                        + AIE_CYCLE
                                        + store_O_S
                            ...
                            temp0_cycle = np.multiply(temp_cycle,
                                                      np.transpose(MODEL_IN[:, 3]))
                            total_cycle = np.sum(temp0_cycle)

                            ########## Accept design if competitive ##########
                            if total_cycle * 0.85 <= best_time:

                                if total_cycle < best_time:
                                    best_time = total_cycle
                                    index = cnt_best % num_design_best
                                    cnt_best += 1
                                else:
                                    index = (num_design_best +
                                             (cnt_choice % num_design_choice))
                                    cnt_choice += 1
    ...
    placement = np.zeros([1, 4])
    col = (50 - Versal_HW_temp[9]) // 2
    row = 0
    placement[0, 1:4] = [col, row, Versal_HW_temp[10]]

    BUFF_SEL = np.zeros([1, 3])
    buff_sel_temp = np.binary_repr(int(Versal_HW_temp[16]), width=3)

    for i in range(3):
        BUFF_SEL[0, i] = 0 if buff_sel_temp[i] == "0" else 1
    ...
    bram_use, uram_use, buf_index = buff_count_0(
        ...
        PACK_IN, PACK_OUT,
        LEFT_SIZE, RIGHT_SIZE, OUT_SIZE,
        Versal_HW[0, 3], Versal_HW[0, 4], Versal_HW[0, 5],
    ...
    return (
        final_config,
        best_cycle,
        config[0, num_term:num_term + sample_num],
        HW_Used
    )
