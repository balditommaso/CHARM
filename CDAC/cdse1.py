...
def cdse1_top(MODEL_IN, DATA_TYPE, part):
    ...
    total_ops = np.sum(
        np.multiply(
            np.multiply(
                np.multiply(MODEL_IN[:, 0], MODEL_IN[:, 1]),
                MODEL_IN[:, 2]
            ),
            MODEL_IN[:, 3]
        )
    ...
    BRAM = math.floor((967 - 199) * portion)  # 100 for AXI bound consumption
    ...
    if DATA_TYPE == 1:
        ...
        PI, PK, PJ = [8, 64, 4]
        mac = 128
        COMPUTE_CYCLE = (PI * PK * PJ // mac + 4) * (H1 // PI) * (W1 // PK) * (W2 // PJ) + 40
        PACK_IN = 2
        PACK_OUT = 2
        kernel_type = 7
        ...
        PI, PK, PJ = [8, 8, 2]
        mac = 8
        COMPUTE_CYCLE = (PI * PK * PJ // mac + 1) * (H1 // PI) * (W1 // PK) * (W2 // PJ) + 40
        PACK_IN = 4
        PACK_OUT = 4
        kernel_type = 1
    ...
    # Data transmission per graph iteration
    ...
    # One-time DDR bandwidth profiling
    ...
    for c in range(1, 8 + 1):  # Row Constraint
        print("DSE Processes ------------ " + str(12.5 * c) + "%")

        for b in range(1, 24 + 1):  # Col Constraint
